...
> **v2.10.7** | 极速加载，实时翻译，本地缓存。由开源爱好者为开源爱好者构建。
...
项目名称、简介、README 一键翻译成中文，**全部免费、无需任何 API Key**，失败时自动回退到下一引擎：
...
- 🧠 **自动选择**：默认引擎优先 Edge（国内直连），失败自动回退 Google → 本地词典，全程无需配置
- 💾 翻译结果走 **SQLite 缓存 + 本地记忆库**（30 天 TTL），二次打开秒开
- ⚡ 切换视图/项目自动取消在途翻译，线程池不拥堵，**翻译 UI 无残留**

### ⚙️ 其他能力

| 功能 | 说明 |
|------|------|
| 📖 项目详情页 | README 中文翻译 + **图片自动嵌入**、语言/Stars/Forks/下载资源一键直达 |
| ⭐ 收藏夹 / 历史 | 数据保存在本地（配置文件），收藏页列表同样支持自动翻译 |
| 🔔 系统托盘 | 点击关闭最小化到托盘，后台常驻，双击恢复 |
...
├── config.json            # 用户配置（Token/密钥经 DPAPI 加密，含收藏与下载历史）
├── app.db                 # SQLite：翻译缓存 / 翻译记忆库 / 排名历史 / 学习词条
└── user_plugins\          # 插件目录
```
...
| **存储** | SQLite（翻译缓存/记忆库/排名历史）+ 本地配置文件（收藏/历史） |
| **打包** | PyInstaller 6.22+，支持 GitHub Actions 自动构建发布 |
| **翻译** | 微软 Edge / Google / 本地术语词典（全免费，无需 Key） |
